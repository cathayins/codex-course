...
The footer label is **Codex Course**. The right side uses the same sans-serif typography as the site and lists only two plain mail links: Course Team (`contact@example.invalid`) and Course Team (`contact@example.invalid`), separated by a neutral `&`. Do not add avatars, role labels such as “講師”, “持續更新”, editorial status, or shared-work-model messaging.
...
- The footer reads “Codex Course” and lists Course Team and Course Team without update-status text.
