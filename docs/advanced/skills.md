...
<section class="lesson-goals" aria-labelledby="skills-basic-goals-title">
  ...
  <div class="lesson-goals__grid">
    <article><strong>判斷使用時機</strong><span>分清楚 Prompt、AGENTS.md、Skill 與 Plugin。</span></article>
...
| 要安裝、分享或連接整組能力 | **Plugin** | 同時安裝信件整理流程與 Gmail connector |
...
<!-- Screenshot asset: /images/skills/skills-sidebar.webp -->
<figure class="skills-screenshot" aria-labelledby="skills-shot-sidebar-title">
  <div class="skills-screenshot__placeholder" role="img" aria-label="Desktop App 側欄 Skills 與詳細資料操作示意">
    <span>APP WALKTHROUGH · BASIC 01</span>
    <strong id="skills-shot-sidebar-title">從側欄瀏覽 Skills</strong>
    <p>先比較 Skill 的名稱與用途，再打開詳細資料，確認它的適用情境與你的任務相符。</p>
  </div>
  <figcaption>實際清單會依 App 版本、帳號、Workspace 政策與已安裝 Plugins 改變。重點是讀懂用途，不是記住固定名稱。</figcaption>
</figure>
...
| 方式 | 怎麼做 | 適合情境 |
| --- | --- | --- |
| **明確呼叫** | 在 Prompt 中加入 `$skill-name` | 剛開始使用、需要確認流程、高風險或要求可重現的任務 |
| **自動選用** | 直接描述任務，讓 Codex 依 Skill 的 description 判斷 | 已熟悉環境、任務意圖非常明確的日常工作 |

剛開始使用時，先練習明確呼叫。你可以直接看到自己選了什麼，也比較容易判斷結果是否真的遵循該 Skill。

<!-- Screenshot asset: /images/skills/skills-composer.webp -->
<figure class="skills-screenshot" aria-labelledby="skills-shot-composer-title">
  <div class="skills-screenshot__placeholder" role="img" aria-label="Desktop App 輸入區使用錢字符號選擇 Skill 的操作示意">
    <span>APP WALKTHROUGH · BASIC 02</span>
    <strong id="skills-shot-composer-title">輸入 <code>$</code> 明確選擇 Skill</strong>
    <p>在輸入區輸入 <code>$</code>，從清單選擇需要的 Skill，接著在名稱後方指出本次任務與必要差異。</p>
  </div>
  <figcaption>在 CLI／IDE 中也能使用 <code>$</code>，並可透過 <code>/skills</code> 瀏覽目前可用的 Skills。</figcaption>
</figure>
...
- [下一章：Plugins](/advanced/plugins) — 把 Skills、連線與擴充元件做成可安裝的能力包。
