...
本章以 [Skills](/advanced/skills) 為基礎，再加入 Apps、connectors 與 tools，組成可以安裝和分享的完整能力包。
...
::: danger 高影響動作保留人工確認
寄信、刪除資料、公開發布、部署與修改 Production 都會影響他人或外部系統。Plugin 提供能力，不代表這些動作應在無人審查下自動執行。首次使用前，先確認 Workspace／Codex 權限，以及 Plugin 使用的外部 tools 與資料範圍。
:::
