...
第一次上手用一般 Prompt 就夠了。如果希望 Codex 建立後持續檢查畫面與限制，可以把同一件事寫成 Goal：

```text
/goal 完成 `tokyo-travel-plan.html`，並確認：
1. 已參考 `tokyo-trip-brief.txt` 與 `tokyo-travel-research.md`，日期、航班、住宿、預約與飲食限制都已呈現。
2. 每天最多只有兩個主要區域；缺少的營業時間、票價與交通細節都標成「出發前確認」。
3. 桌面與手機沒有水平溢出，切換按鈕可以操作，列印版能閱讀。

不得修改來源檔；外部補充資訊要註明來源與查詢日期，
還不能確認的內容則標成「出發前確認」。
若無法安全繼續，請停止並說明還缺少什麼資訊。
```
