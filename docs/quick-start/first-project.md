---
title: First Project｜開始第一個 Codex 任務
description: 建立工作資料夾並在 Codex 開啟，選擇 Model，再送出第一個 Message。
outline: [2, 3]
aside: true
pageClass: quickstart-story first-project-page
---

# First Project｜開始第一個 Codex 任務

<p class="lesson-lead">第一次使用 Codex，只要完成三件事：準備工作資料夾、選擇 Model，再說明你想完成什麼。先走完一次，後面再補 Prompt 與進階操作。</p>

<FirstProjectDemo mode="overview" />

## Step 1｜創建資料夾並於 Codex 打開

先在 Finder 或檔案總管建立 `codex-quickstart`。這個資料夾就是 Codex 這次讀取資料、修改檔案與存放成果的範圍。

開啟 Codex App，在「專案」旁按 **＋**，選擇 **使用現有資料夾**，再選取剛建立的 `codex-quickstart`。

<FirstProjectDemo mode="folder" />

::: tip 資料夾可以先是空的
你可以先建立空資料夾，請 Codex 從零建立檔案；也可以先放入這次會用到的文件。練習時不要放客戶個資、密碼或公司機密。
:::

## Step 2｜選擇 Model

在輸入框下方點選 Model 名稱，再依任務需求切換。第一次練習可以先沿用畫面的預設 Model；名稱與可用選項以你的 Workspace 畫面為準。

<FirstProjectDemo mode="model" />

模型會影響速度、推理能力與用量，但不會替你補上缺少的工作目標。下一頁會再說明不同 Model 的適用情境。

## Step 3｜Send your first message

先用一句話說清楚目標，再補上 Codex 需要的檔案、背景或限制。第一個 Message 不必完美，看完結果後可以繼續補充。

<FirstProjectDemo mode="message" />

建議第一次先請 Codex 盤點資料夾，不要修改檔案。確認它找對工作範圍後，再請它分析內容或建立成品。

## 接下來怎麼走

- [選擇 Model](/quick-start/models)：比較不同 Model 與 reasoning effort。
- [Prompting](/quick-start/prompting)：補上來源、成品格式與必要限制。
- [Follow Up](/quick-start/follow-up)：用後續訊息調整第一版結果。

## 參考資料

- [ChatGPT Quickstart](https://learn.chatgpt.com/docs/quickstart)
- [Projects and chats](https://learn.chatgpt.com/docs/projects)
- [Prompting](https://learn.chatgpt.com/docs/prompting)
